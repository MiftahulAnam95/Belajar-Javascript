window.JSLabApp = (() => {
  const data = window.JSLabData;
  const progress = window.JSLabProgress;
  ...
  const LESSON_RECALL_STORAGE_KEY = "javascript-beginner-lab-lesson-recall-v1";
  const RECALL_STORAGE_KEY = "javascript-beginner-lab-recall-answers-v1";
  const DEBUG_ATTEMPT_STORAGE_KEY = "javascript-beginner-lab-debug-attempts-v1";
  ...
  let activeEditorDebugJsOnly = false;
  ...
  const highlightCode = (line = "", filename = "") => {
    if (filename.endsWith(".html")) return highlightHTML(line);
    return escapeHTML(line)
      .replace(/\b(const|let|var|function|return|if|else|for|while|document|querySelector|getElementById|addEventListener)\b/g, '<span class="tok-tag">$1</span>')
      .replace(/(&quot;.*?&quot;|&#039;.*?&#039;|`.*?`)/g, '<span class="tok-string">$1</span>')
      .replace(/(\/\/.*)$/g, '<span class="tok-comment">$1</span>');
  };

  const renderCodeBlock = (code, filename = "index.html") => {
    const lines = String(code).split("\n").map((line) => highlightCode(line, filename));
    return `
      <div class="code-card">
  ...
  const buildPreviewDocument = (code = "", css = "", js = "") => {
    const scriptBlock = js ? `<script>${js.replace(/<\/script/gi, "<\\/script")}<\/script>` : "";
    const isFullDocument = /<html[\s>]/i.test(code);
    if (isFullDocument) {
      if (!css && !js) return code;
      const styleBlock = `<style>${css}</style>`;
      const withStyle = css
        ? /<\/head>/i.test(code)
          ? code.replace(/<\/head>/i, `${styleBlock}</head>`)
          : code.replace(/<body[\s>]/i, `${styleBlock}$&`)
        : code;
      return /<\/body>/i.test(withStyle) ? withStyle.replace(/<\/body>/i, `${scriptBlock}</body>`) : `${withStyle}${scriptBlock}`;
    }
    return `<!DOCTYPE html>
<html lang="id">
  <head>
  ...
  <body>${code}${scriptBlock}</body>
</html>`;
  };

  const debugPreviewHtml = `<main class="card" id="app">
  <h1>Preview Debugging JavaScript</h1>
  <p id="pesan">Klik tombol atau isi form untuk melihat efek script.</p>
  <button id="tombol">Klik tombol</button>
  <form id="form" class="debug-form">
    <label for="nama">Nama</label>
    <input id="nama" placeholder="Rina">
    <label for="inputA">Angka A</label>
    <input id="inputA" value="2">
    <label for="inputB">Angka B</label>
    <input id="inputB" value="3">
    <button type="submit">Cek form</button>
  </form>
  <p id="hasil"></p>
</main>`;

  const debugPreviewCss = `body {
  background: #fffbeb;
  font-family: Arial, sans-serif;
}
...
input {
  ...
  const getDebugPreviewParts = (code = "") => {
    ...
    if (!source) return { css: debugPreviewCss, html: debugPreviewHtml, js: "", jsOnly: true };
    const hasHtml = /<\/?[a-z][\s\S]*?>/i.test(source);
    if (hasHtml) return { css: "", html: source, js: "", jsOnly: false };
    return { css: debugPreviewCss, html: debugPreviewHtml, js: source, jsOnly: true };
  };
  ...
  const runDebugAnswerPreview = () => {
    ...
    previewFrame.srcdoc = buildPreviewDocument(parts.html, parts.css, parts.js);
  };
  ...
  const renderLessonCodePreview = (item) => `
    <div class="lesson-code-preview">
      <div class="lesson-preview-label"><i class="bi bi-eye"></i> Preview hasil HTML + CSS + JavaScript</div>
      <iframe
        class="lesson-preview-frame"
        title="Preview contoh kode ${escapeHTML(item.title)}"
        sandbox="allow-scripts"
        loading="lazy"
        srcdoc="${escapeHTML(buildPreviewDocument(item.previewHtml || item.html || item.code, item.previewCss || item.css || "", item.previewJs || item.js || ""))}"
      ></iframe>
    </div>`;
  ...
  const renderLessonCodeBlocks = (item) => {
    ...
    if (item.js) blocks.push(renderCodeBlock(item.js, "script.js"));
  ...
  const openLesson = (id, updateHash = true) => {
    if (document.body.dataset.page !== "materi" || !getElement("lessonDetail")) {
  ...
  const renderDebuggingChallenge = (id = activeDebugId) => {
    ...
    detail.innerHTML = `
      <span class="eyebrow"><i class="bi bi-bug"></i> Debugging JavaScript</span>
      <h2>${escapeHTML(item.title)}</h2>
      ...
      <form class="mt-3" data-debug-form="${item.id}">
        ...
        <div class="debug-editor-grid mt-3">
          ...
          <div class="debug-preview-shell">
            ...
            <iframe class="debug-preview-frame" id="debugPreviewFrame" title="Preview jawaban debugging ${escapeHTML(item.title)}" sandbox="allow-scripts"></iframe>
          </div>
        </div>
        <div class="lesson-actions mt-3">
  ...
  const initCssColorPicker = (cssInput, onChange) => {
    ...
    const presets = ["#FFFFFF", "#111827", "#F59E0B", "#B45309", "#16A36A", "#0EA5B7", "#7C3AED", "#DC4C64", "#FBBF24", "#22C55E", "#38BDF8", "#A855F7"];
  ...
  const setEditorDebugChallenge = (id) => {
    ...
    if (!challenge || !htmlInput || !cssInput || !jsInput) return;
    ...
    activeEditorDebugJsOnly = parts.jsOnly;
    htmlInput.value = parts.html;
    cssInput.value = parts.css;
    jsInput.value = parts.js;
  ...
  const saveEditorDebugDraft = () => {
    ...
    const code = activeEditorDebugJsOnly
      ? jsInput.value
      : [
          htmlInput.value.trim(),
          cssInput.value.trim() ? `<style>\n${cssInput.value.trim()}\n</style>` : "",
          jsInput.value.trim() ? `<script>\n${jsInput.value.trim()}\n</script>` : ""
        ]
          .filter(Boolean)
          .join("\n\n");
  ...
  const runEditorPreview = () => {
    ...
    previewFrame.srcdoc = buildPreviewDocument(htmlInput.value, cssInput.value, jsInput.value);
  };
  ...
  const initEditor = () => {
    ...
    if (!htmlInput || !cssInput || !jsInput || !templateButtons) return;
    ...
    editorUndoHistory = initEditorUndoHistory([htmlInput, cssInput, jsInput], runEditorPreview);
    initEditorTabBehavior([htmlInput, cssInput, jsInput], runEditorPreview);
  ...
  const handleHashLesson = () => {
    ...
    if (lessonId && data.lessons.some((lesson) => lesson.id === lessonId)) {
      ...
      if (lesson) document.title = `${lesson.title} - JavaScript Beginner Lab`;
      return;
    }
...
document.addEventListener("DOMContentLoaded", window.JSLabApp.init);
